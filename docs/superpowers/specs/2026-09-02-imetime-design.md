...
- 狀態：待專案擁有者最終確認後進入 writing-plans
- 產品名稱：**ImeTime**
...
5. 10 名朋友每天使用的雲端月費在第一年 **接近 NT$0**，上限 NT$1,000（見 §3.2 成本模型）。
...
- **500 MB DB** → 每年約 3.5 萬筆 clips + 訊息，遠低於上限。

### 3.2 影片物件儲存與成本模型

假設：10 名成員、每天約 **96 支**（24 格 × 10 人 × 40% 參與率）；每支 2 秒 1080p HEVC 約 2 MB（含海報圖）。
...
| 方案 | 免費額度 | 超額單價 | 第一年月費估算 | 備註 |
|---|---|---|---|---|
| **Cloudflare R2** | 10 GB 儲存永久免費；**出口流量永遠免費** | US$0.015/GB-月 | US$0 → 年末約 US$0.9 | 啟用需在帳戶綁卡，免費範圍內不扣款；S3 相容 API |
| AWS S3 | 5 GB 永久免費；全 AWS 每月前 100 GB 出口免費；新帳戶 6 個月 US$100–200 抵用金 | 儲存約 US$0.025/GB-月（東京）；出口超過 100 GB 後約 US$0.114/GB | US$0 → 年末約 US$1.6 | 若朋友重看多、流量超過 100 GB/月即開始收費 |
| Supabase Storage（Pro） | 100 GB / 250 GB 出口 | 含在 US$25/月 | US$25 | 需付費方案 |

**決策**：影片放 **Cloudflare R2**。R2 儲存單價比 S3 低、且無出口流量懸崖，是這個用量下最便宜的選項。R2 與 S3 使用同一套 S3 API 與 SigV4 簽名，程式碼完全相同；若日後想改 AWS S3，只需更換 Edge Function 的 endpoint 與金鑰。App 端一律透過 Edge Function 簽發的短效 presigned URL 上傳與下載，不知道也不在乎後面是哪家。
...
- 「有人發了片段」使用 `apns-collapse-id = room_id`：同房間的多則通知在通知中心合併為一則並更新內文（「小明、小華 發了新片段」）。
...
| # | 決策 | 結果 |
|---|---|---|
| D1 | 後端與儲存 | Supabase Free（metadata/認證/即時）+ Cloudflare R2（影片，S3 相容，可隨時改 AWS S3） |
