...
make test
```
Expected：`test-core`（Core 全部）、`test-db`（7 個 pgTAP 檔）、`test-app` 全綠。
...
- **Spec 覆蓋**：§2 規則 1（Task 1–3）、規則 13 的「離開者不再可讀」（Task 2 RLS）、§5 rooms/room_members 與 `abandoned_at`（Task 2）、§5.1 三個 RPC 與速率限制（Task 3）、§5.2 rooms/room_members/profiles 政策（Task 2）、§6.2（Task 7、8）、§14 畫面 4、5、6、13（Task 6–9）。`delete_account()` 依 spec 屬 P6。
